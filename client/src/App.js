...
import { getSightings, getSighting } from "./SightingService";
...
function App() {
  ...
  const updateSighting = (sighting_id) => {
    const temp = birdSightings.map(s => s);
    const indexToUpdate = temp.map(s => s._id).indexOf(sighting_id);
    getSighting(sighting_id).then((sighting) => {
      temp[indexToUpdate] = sighting;
      setBirdSightings(temp);
    })
  }
